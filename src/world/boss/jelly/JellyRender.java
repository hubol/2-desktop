...
public class JellyRender extends Entity{
	public static JellyRender me;
	...
	public JellyRender(double x, double y) {
		super(x, y);
		setDepth(-8);
	...
	public void render(){
		...
		for (int p=0; p<draws; p++){
			...
			//nonenemy
			ArrayList<Jelly> list = Scene.getEntityList(Jelly.class);
			for (int i=0; i<list.size(); i++){
				Jelly me = list.get(i);
				if (me.visible && !JellyEnemy.class.isInstance(me)){
			...
			//enemy
			ArrayList<JellyEnemy> bist = Scene.getEntityList(JellyEnemy.class);
			for (int i=0; i<bist.size(); i++){
				JellyEnemy me = bist.get(i);
				if (me.visible){
					double xx = me.x, yy = me.y, xs = me.xscale, ys = me.yscale, an = me.angle, alp = me.alpha;
					me.x += addX;
					me.y += addY;
